...
const AddUserDialog: FunctionComponent<AddUserDialogProps> = () => {
    ...
    const onSubmit = async (user: IUser) => {
        try {
            ...
            if (serverMessage === "Email already taken") {
                alert("שגיאה: כתובת האימייל הזו כבר תפוסה במערכת");
            } else {
                alert("שגיאה בהוספת המשתמש, בדוק את הפרטים");
            }
        }
        finally {
    ...
    return (
        ...
        >
            <DialogTitle sx={{ fontFamily: customFont, fontWeight: 800, textAlign: 'center' }}>
